...
const Wrapper = ({
    ...
    variant === 'card' ? (
        ...
        <Box sx={{ borderTop: `1px solid ${hexAlpha(accent, 0.15)}`, p: 1.25 }}>{children}</Box>
    );
...
export default function ClassificationReviewCard({
    ...
    if (review.wrong_upload_area) {
                ...
                >
                    ...
                    >
                        ...
                        >
                            {overrideOpen
                                ? '// CANCELAR CORRECCION'
                                : '// CLASIFICACION INCORRECTA — CORREGIR MANUALMENTE'}
                        </Typography>
                    </Box>
        ...
        return (
            <Wrapper accent={palette.amber} variant={variant}>
                {wrongAreaContent}
            </Wrapper>
        );
    }
    ...
    return (
        <Wrapper accent={moduleAccents.upload} variant={variant}>
            {normalContent}
        </Wrapper>
    );
}
